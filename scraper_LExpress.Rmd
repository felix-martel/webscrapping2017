...
relevant_words = colnames(lexpress_words_table)[3:ncol(lexpress_words_table)]

lexpress_words_hotness <- lexpress_words_table %>% 
  group_by(date) %>% 
  summarise_at(relevant_words,sum)

ggplot(lexpress_words_hotness)+aes(x=date)+ylab("hotness")+
...
non_zero_rows <- which(lexpress_words_table %>% select(-title,-date) %>% rowSums() >0)
lexpress_words_table_no_zero_rows <- lexpress_words_table[non_zero_lines,]

lexpress.CA <- CA(lexpress_words_table_no_zero_lines,quali.sup=c(1,2),graph=F)
